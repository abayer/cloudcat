			...
			<table>
				...
				<g:each in="${reportRunList}" status="i" var="reportRunInfo">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                                          <td><g:link action="report" id="${reportRunInfo.id}">${fieldValue(bean: reportRunInfo, field: "id")}</g:link></td>
                                          <td><g:link action="report" id="${reportRunInfo.id}"><g:formatDate date="${reportRunInfo.dateCreated}" /></g:link></td>
					</tr>
				</g:each>
				</tbody>
			</table>
